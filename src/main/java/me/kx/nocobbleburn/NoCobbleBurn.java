...
public final class NoCobbleBurn extends JavaPlugin implements Listener {
	...
	public void onEntityBurn(EntityCombustByBlockEvent e) {
		...
		if (!genCobblestone.contains(item)) return;
		Block combuster = e.getCombuster();
		if(combuster == null || combuster.getType() != Material.LAVA) return;

		ItemStack itemStack = item.getItemStack();
		if(itemStack.getType() != Material.COBBLESTONE || itemStack.getType() != Material.STONE) return;
		e.setCancelled(true);
	}
}
